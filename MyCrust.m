...
tic
tetr=delaunayn(data);%creating tedraedron
...
%% Circumcenters(Nested)
    function [cc,r]=CC()
        ...
        v21=data(tetr(:,1),:)-data(tetr(:,2),:);
        v31=data(tetr(:,3),:)-data(tetr(:,1),:);
        v41=data(tetr(:,4),:)-data(tetr(:,1),:);
...
%% Connectivity(Nested)
    ...
    function [t2tetr,tetr2t]=Connectivity(tetr)
        ...
        t2tetr = [j(vect), j(vect+numt), j(vect+2*numt),j(vect+3*numt)];%each tetraemdro has 3 triangles
...
%% Walking(Nested)
    function t=Walking()
        np=size(data,1)-540;%540 = number of shield points put at the end of array
        numtetr=size(tetr,1);
        nt=size(tetr2t,1);
